...
\subsection{Approximate nearest-neighbor families}
...
The literature reviewed above supplies mature answers for unconstrained ANN, but it leaves a gap in workload-aware subset-constrained search. Exact multidimensional indexes teach how to prune by structure, ANN methods teach how to trade precision for speed, and graph- or quantization-based systems show how to scale. Yet the standard formulations do not directly answer when an ANN system should search globally, when it should route to a constrained partition, or when it should abandon ANN altogether and use exact local evaluation.

This report positions temporal subset ANN as a \emph{strategy selection} problem. The prototype does not claim a universally best index. Instead, it tests whether different search organizations dominate under different temporal workloads, and whether a planner can identify those regimes reliably. That framing connects the literature review directly to the experimental component of the report.
...
\[
S_q = \{r_i : [t_i^{start}, t_i^{end}] \cap [T_s, T_e] \neq \emptyset,\; p_i \in [P_s,P_e],\; c_i \text{ matches if specified}\},
\]
